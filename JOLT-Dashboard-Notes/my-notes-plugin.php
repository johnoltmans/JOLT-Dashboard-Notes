...
function register_notes_cpt() {
    $labels = [
        'name' => 'JOLT Notes', // Terug naar origineel, versie komt op settings pagina
        'singular_name' => 'Note',
        'add_new' => 'Add New',
        ...
        'menu_name' => 'JOLT Notes' // Terug naar origineel, versie komt op settings pagina
    ];
...
// Voeg aangepaste admin-pagina toe voor alleen-lezen weergave (voor individuele notities)
function add_note_view_page() {
    add_submenu_page(
        null, // Maak deze pagina een "sub-sub-menu" zodat hij niet direct zichtbaar is in de navigatie
        'View Note',
        'View Note',
        'edit_posts',
        'view-note',
...
// *** NIEUWE CODE VOOR DE SETTINGS PAGINA ***

// Voeg de settings pagina toe aan het JOLT Notes menu
function jolt_notes_add_settings_page() {
    add_submenu_page(
        'edit.php?post_type=note', // Parent slug: voeg toe onder het JOLT Notes menu
        'JOLT Notes Settings',     // Pagina titel
        'About',                // Menu titel
        'manage_options',          // Vereiste rechten
        'jolt-notes-settings',     // Unieke slug voor deze pagina
        'jolt_notes_render_settings_page' // Callback functie om de inhoud te renderen
    );
}
add_action('admin_menu', 'jolt_notes_add_settings_page');

// Render de inhoud van de settings pagina
function jolt_notes_render_settings_page() {
    // Get the current plugin version from the plugin header
    $plugin_data = get_plugin_data(__FILE__);
    $version = $plugin_data['Version'];
    ?>
    <div class="wrap">
        <h1>JOLT Dashboard Notes v<?php echo esc_html($version); ?></h1>
        <p>This is the settings page for JOLT Dashboard Notes.</p>
        <p>Future settings for this plugin will appear here.</p>
        </div>
    <?php
}
